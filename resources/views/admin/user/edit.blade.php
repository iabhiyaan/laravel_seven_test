<x-admin-layout title="User create">
    ...
    <div class="container-fluid">
        @if (count($errors) > 0)
            <x-error-message />
        @endif
        <form action="{{ route('user.update', $detail->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-9">
                            ...
                            <div class="form-group col-12">
                                <div class="custom-control custom-checkbox mb-2">
                                    <input type="checkbox" class="custom-control-input" id="is_published"
                                        ...
                                        {{ $detail->is_published == 1 ? 'checked' : null }}>
                            ...
                            @foreach ($access_options as $key => $option)
                                <div class="custom-control custom-checkbox mb-2">
                                    <input type="checkbox" class="custom-control-input" id="{{ $key }}"
                                        ...
                                        {{ in_array($key, $oldAccesses) ? 'checked' : null }}>
